...
```
1. Legacy/internal system creates an authorization-capable session
2. Customer completes authorization via Commerce Payments
   → status: "authorized", authId set
3. Merchant reviews order
4. Merchant captures payment
   → POST /api/console/payments/capture
   → status: "confirmed"

   OR voids the authorization:
...
```
pending → authorized → confirmed   (authorized → captured)
                     → voided      (authorization released)
         → failed                  (authorization declined)
         → expired                 (session timed out before auth)
```
