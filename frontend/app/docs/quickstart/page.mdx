...
```
POST /api/checkouts
  Headers: { "x-api-key": "coal_live_..." }
  Body: { amount: 25, productName: "Premium Membership", redirectUrl: "https://yoursite.com/success" }
  → Returns: id, url, status, amount, currency, expiresAt

POST /api/pay/confirm
  Body: { sessionId, txHash }
  → Returns: { status: "verifying", sessionId }

GET /api/pay/status/:sessionId
  → Returns: { status, txHash, redirectUrl }
```
